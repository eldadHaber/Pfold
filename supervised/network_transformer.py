...
def tr2DistSmall(Y):
    ...
    print(torch.cuda.memory_summary())
    # D = torch.repeat_interleave(torch.sum(Z**2, dim=1).unsqueeze(1),Z.shape[-1],dim=1) + torch.repeat_interleave(torch.sum(Z**2, dim=1).unsqueeze(2),Z.shape[-1],dim=2) - 2*Z.transpose(1,2) @ Z
    D = 3*D/k
