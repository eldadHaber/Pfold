...
def find_chain_fit(models_cd,seq,AA_shortener,AA_convert):
    ...
    for model_cd in models_cd.items():
        aa = model_cd[1].child_list
        seq_pdb_long = [AA_shortener.get(i.resname, 'X') for i in aa]
        seq_pdb = AA_convert(seq_pdb_long).squeeze().numpy()
        if np.sum(seq_pdb != -1) == 0:
            continue
        seq_pdb = remove_leading_and_trailing_values(seq_pdb,-1)
        seq_pdb_char = AA_convert.reverse(seq_pdb)
        seq_char = AA_convert.reverse(seq)
        seq1 = Seq(seq_char)
        seq2 = Seq(seq_pdb_char)
...
def DSSP_wrapper2(file,DSSP_ALPHABET,seq,chain_number,chain_id):
    ...
    unk_tok = 'X'
    models_cd = models.child_dict
    AA_convert = AA_converter(AA_alphabet, unk_idx=unk_idx, unk_tok=unk_tok)
    ...
    if chain_id == -1:
        chain_id = find_chain_fit(models_cd, seq, AA_shortener, AA_convert)
        to_use = np.asarray([i == chain_id for i in chain_ids])
...
def find_pdb_id(seq):
    ...
    print("Using blast to find sequence pdb_id")
    ...
    for alignment in blast_record.alignments:
        for hsp in alignment.hsps:
            if hsp.query == seq and hsp.match == seq and hsp.sbjct == seq:
                match_found = True
                pdb_id = alignment.accession.split('_')[0]
                chain_id = alignment.accession.split('_')[1]
                chain_number = alignment.accession.split('_')[2]
                break
        if match_found:
            print("pdb_id found")
            break
    return pdb_id, chain_id, chain_number


def add_dssp_to_file(file_in):
    t1 = time.time()
    d = np.load(file_in)
    seq = d['seq']
    id = str(d['id'])
    id = Path(file_in).stem
    pdb_id = id.split('#')[-1].split('_')[0]
    file_pdb = "{:}{:}.cif".format(pdb_folder,pdb_id.lower())
    if not Path(file_pdb).is_file():
    ...
    if Path(file_pdb).is_file():
        dssp_num, dssp_success = DSSP_wrapper2(file_pdb,DSSP_ALPHABET,seq,chain_number,chain_id)
    else:
        converter = AA_converter(aa_list=AA_alphabet,unk_idx=-1)
        pdb_id, chain_number, chain_id = find_pdb_id(converter.reverse(seq))
    ...
    if dssp_success:
        ...
        file_out = "{:}{:}.npz".format(reject_folder, Path(file_in).stem)
...
if __name__ == '__main__':
    ...
    # pdb_folder = './pdbs/'
    #
    pnet_folder = '/media/tue/Data/Dropbox/ComputationalGenetics/data/pnet_with_msa_training95/'
    output_folder = '/media/tue/Data/Dropbox/ComputationalGenetics/data/pnet_with_msa_and_dssp_training95/'
    reject_folder = '/media/tue/Data/Dropbox/ComputationalGenetics/data/rejected_pnet_with_msa_and_dssp_training95/'
    ...
    search_command = output_folder + "*.npz"
    out_files = [f for f in sorted(glob.glob(search_command))]
    out_basefiles = [Path(x).stem for x in out_files]

    search_command = reject_folder + "*.npz"
    reject_files = [f for f in sorted(glob.glob(search_command))]
    reject_basefiles = [Path(x).stem for x in reject_files]

    pnet_files_todo = [x for x in pnet_files if Path(x).stem not in (out_basefiles + reject_basefiles)]

    print("Total number of files={:}, already done={:}, rejected={:}, remaining={:}".format(npnet_files,len(out_files),len(reject_basefiles),len(pnet_files_todo)))
    t0 = time.time()

    # with mp.Pool(processes=4) as pool:
    #     for i in pool.imap_unordered(add_dssp_to_file, pnet_files_todo):
    #         pass
    #     pool.close()
    #     pool.join()

    for i,pnet_file_todo in enumerate(pnet_files_todo):
        add_dssp_to_file(pnet_file_todo)
        print("{:}/{:} time taken={:2.2f}h, ETA={:2.2f}h".format(i+1,len(pnet_files_todo),(time.time()-t0)/3600,((time.time()-t0)/3600)/(i+1)*(len(pnet_files_todo)-i-1)))