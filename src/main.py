...
def main(c):
    ...
    net = train(net, optimizer, dl_train, loss_fnc, c.LOG, device=c.device, dl_test=dl_test, max_iter=c.max_iter, report_iter=c.report_iter, scheduler=scheduler, sigma=c.sigma, checkpoint=c.checkpoint, save="{:}/".format(c.result_dir), use_loss_coord=c.use_loss_coord)
