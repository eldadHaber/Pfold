...
if __name__ == "__main__":
    ...
    for i, a2mfile in enumerate(a2mfiles):
        ...
        if np.sum(res) == 1:
            ...
            txn.put(u'{}'.format(cnt).encode('ascii'), dumps_pyarrow((protein, pssm, f2d_dca, r1[org_id].T, r2[org_id].T, r3[org_id].T)))
            if (cnt + 1) % write_freq == 0:
                # print("[%d]" % (cnt + 1))
                txn.commit()
                txn = db.begin(write=True)
            if (cnt + 1) % report_freq == 0:
                print("loading sample: {:}, excluded: {:}, problems: {:}  Time(read): {:2.2f}, Time(lookup): {:2.2f}, Time(ANN): {:2.2f}, Time(DCA): {:2.2f}, Time(total): {:2.2f}".format(cnt + 1, n_excluded,
                                                                                               problems,avg_read_time/(i+1-n_excluded),avg_lookup_time/(i+1-n_excluded),avg_ann_time/(n_matches),avg_dca_time/(n_matches),time.time() - t0))
            cnt += 1
    ...
    db.sync()
    db.set_mapsize(
        1)  # This basically just sets the mapsize as small as possible after we have populated the database.
    db.close()
